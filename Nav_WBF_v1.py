...
while True:
    ...
    for i in range(len(boxes_depth)):
        if i in indexes:
            ...
            cv2.circle(depth_colormap, (int(x), int(y + h)), radius=30, color=(255, 255, 0), thickness=-1)
            cv2.circle(depth_colormap, (int(x + w), int(y + h)), radius=30, color=(255, 255, 0), thickness=-1)
