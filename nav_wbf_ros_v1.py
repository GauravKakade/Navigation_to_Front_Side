...
def image_callback_depth(depth_frame):
    #print("Received a depth image!")
    try:
        # Convert your ROS Image message to OpenCV2
        cv2_img = bridge.imgmsg_to_cv2(depth_frame, "passthrough")
        depth_array = np.array(cv2_img, dtype=np.float32)
        #distance_in_mm = depth_array(x, y) #depth_array(x, y)
        ##center_idx = np.array(depth_array.shape) / 2
        ##print('center depth:', depth_array[center_idx[0], center_idx[1]])
        #depth_colormap = cv2.applyColorMap(cv2.convertScaleAbs(cv2_img, alpha=0.03), cv2.COLORMAP_JET)
        depth_image = np.asanyarray(depth_array)
...
rospy.Subscriber("/camera/depth/image_rect_raw", Image, image_callback_depth)
rospy.Subscriber("/camera/color/image_raw", Image, image_callback_rgb)
# Spin until ctrl + c
rospy.spin()
